...
namespace OnionArcExample.WebAPI
{
    ...
    public class AuthorController : ControllerBase
    {
        ...
        [HttpGet("GetAuthorforAccount")]
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public virtual async Task<IActionResult> GetAuthorForAccount(int id)
        {
            var result = await mediator.Send(new GetAuthorForAccountQuery { Id = id});
            return Ok(result);
        }
    }
}
